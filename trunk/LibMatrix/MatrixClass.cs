...
namespace Mehroz
{
    ...
    public class Matrix
    {
        ...
        {
            if (this.Cols != ((Matrix)obj).Cols)
                return false;

            if (this.Rows != ((Matrix)obj).Rows)
                return false;

            for (int i = 0; i < this.Rows; i++)
                for (int j = 0; j < this.Cols; j++)
                    if (!this[i, j].Equals(((Matrix)obj)[i, j]))
                        return false; //Si encuentra alguna desigualdad return false
    ...
    public class Fraction
    {
        ...
        public Fraction(double iNumerator, long iDenominator)
        {
        ...
        /// </summary>
        private void Initialize(double iNumerator, long iDenominator)
        {
        ...
        public static Fraction Inverse(Fraction frac1)
        {
            ...
            long iDenominator = (long)frac1.Numerator;
            return (new Fraction(iNumerator, iDenominator));
        }
        ...
        public override int GetHashCode()
        {
            return (Convert.ToInt32((((long)Numerator) ^ Denominator) & 0xFFFFFFFF));
        }
        ...
        {
            double iNumerator = -frac1.Numerator;
        ...
        private static Fraction Add(Fraction frac1, Fraction frac2)
        {
            try
            {
                checked
                {
                    double iNumerator = frac1.Numerator * frac2.Denominator + frac2.Numerator * frac1.Denominator;
        ...
        private static Fraction Multiply(Fraction frac1, Fraction frac2)
        {
            try
            {
                checked
                {
                    double iNumerator = frac1.Numerator * frac2.Numerator;
                    long iDenominator = frac1.Denominator * frac2.Denominator;
                    return (new Fraction(iNumerator, iDenominator));
                }
            }
        ...
        /// </summary>
        private static double GCD(double iNo1, double iNo2)
        {
            ...
            do
            {
                if (iNo1 < iNo2)
                {
                    double tmp = iNo1;  // swap the two operands
                    iNo1 = iNo2;
                    iNo2 = tmp;
                }
                iNo1 = iNo1 % iNo2;
        ...
        public static void ReduceFraction(Fraction frac)
        {
            try
            {
                ...
                double iGCD = GCD(frac.Numerator, frac.Denominator);
                frac.Numerator /= iGCD;
                frac.Denominator /= (long)iGCD; // modificado 11/06/06
